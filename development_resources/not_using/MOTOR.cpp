...
*       Sets motor speed from values between 0 and MAXSERVOPWM - aMax
*/
...
void MOTOR::calibrateFromChannels(PCA9685 *chip1, uint8_t channelStart, uint8_t channelEnd)
{
	...
	std::printf("Set %d ESCs to maximum (%d)\n", (channelEnd - channelStart), MAXSERVOPWM);
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));

	for( uint8_t index = channelStart; index <= channelEnd; index++ )
		chip1->setPWM(index, MINSERVOPWM);

	std::printf("Set %d ESCs to minimum(%d)\n", (channelEnd - channelStart), MINSERVOPWM);
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));

	std::printf("Calibration and Arming complete\n");
